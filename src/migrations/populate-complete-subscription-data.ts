  ...
  try {
    ...
    if (existingPlans.length > 0) {
      const backupCollection = db.collection(
        'subscription_plans_backup_' + new Date().getTime(),
      );
    ...
    for (const plan of SUBSCRIPTION_PLANS) {
      try {
        ...
        if (existing) {
          ...
          await collection.replaceOne({ planId: plan.planId }, updateData);
    ...
    // Display all plans
    const allPlans = await collection
      .find({})
    ...
    allPlans.forEach((plan) => {
      ...
      console.log(
        `  Price: $${plan.pricing.baseAmount.toFixed(2)} ${plan.pricing.currency}`,
      );
      ...
      if (plan.conditionalPricing && plan.conditionalPricing.length > 0) {
        console.log(
          `  Conditional Pricing: ${JSON.stringify(plan.conditionalPricing)}`,
        );
      }
    });
  } catch (error) {
